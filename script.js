function init() {
  ...
  myDiagram.nodeTemplate = new go.Node(go.Panel.Spot, {
    ...
    mouseEnter: (e, node) =>
      (node.findObject("BUTTON").opacity = node.findObject("BUTTONX").opacity =
        1),
    mouseLeave: (e, node) => {
  ...
  myDiagram.nodeTemplate.contextMenu = go.GraphObject.build("ContextMenu").add(
    ...
    go.GraphObject.build("ContextMenuButton", {
      click: (e, button) => {
        ...
        if (node !== null) {
          myDiagram.commit(
            (d) => d.removeParts(node.findTreeParts()),
            "remove dept"
          );
        }
      },
...
function save() {
    ...
    const fullJson = myDiagram.model.toJson();
    
    // Parse it into an object
    const fullObject = JSON.parse(fullJson);
    
    // Extract only the nodeDataArray part
    const nodeDataArray = fullObject.nodeDataArray;
    
    // Convert back to JSON and set the value of the textarea
    document.getElementById("mySavedModel").value = JSON.stringify(nodeDataArray, null, 2);
    myDiagram.isModified = false;
}

function load() {
    // Get the content from the textarea and parse it
    const nodeDataArrayJson = document.getElementById("mySavedModel").value;
    const nodeDataArray = JSON.parse(nodeDataArrayJson);
    
    // Wrap the array in the necessary object structure
    const fullObject = {
        "class": "go.TreeModel",
        "nodeDataArray": nodeDataArray
    };
    
    // Convert to JSON and set as the model of the diagram
    myDiagram.model = go.Model.fromJson(fullObject);
    
    // Set up the unique key function
    let lastkey = 1;
    myDiagram.model.makeUniqueKeyFunction = (model, data) => {
        let k = data.key || lastkey;
        while (model.findNodeDataForKey(k)) k++;
...
function changeTheme() {
  ...
  if (myDiagram) {
    myDiagram.themeManager.currentTheme =
